...
def test_minimal_kit_has_exactly_five_files_with_verified_hashes():
...
def test_documented_urls_match_rdl_parameters_and_disable_details():
    ...
    for url in urls:
        ...
        assert parameters['IncludePseudonymizedDetails'] == ['0']
        assert parameters['DataThroughConfirmed'] == ['0']
...
def test_first_test_form_is_offline_and_not_an_analysis_profile():
    ...
    assert "schema_version:'cooperation-1'" in html
    assert not re.search(r'<(?:script|link)[^>]+(?:src|href)="https?://', html)
    assert 'IncludePseudonymizedDetails:' in html and "IncludePseudonymizedDetails:'0'" in html
