SET NOCOUNT ON;
IF @IncludePseudonymizedDetails=0 OR COL_LENGTH(N'DWH.FactPatientImage',N'DimPatientID') IS NULL OR COL_LENGTH(N'DWH.FactPatientImage',N'DimMachineID') IS NULL OR COL_LENGTH(N'DWH.FactPatientImage',N'ImageCreationDate') IS NULL OR COL_LENGTH(N'DWH.DimPatient',N'DimPatientID') IS NULL OR COL_LENGTH(N'DWH.DimPatient',N'IsMOTestPatient') IS NULL OR COL_LENGTH(N'DWH.DimMachine',N'DimMachineID') IS NULL OR COL_LENGTH(N'DWH.DimMachine',N'MachineId') IS NULL
...
DECLARE @image_sql nvarchar(max)=N'INSERT INTO #Images SELECT ' + N'TRY_CONVERT(bigint,s.[DimPatientID])' + N', ' + N'TRY_CONVERT(bigint,s.[DimMachineID])' + N', ' + N'TRY_CONVERT(datetime2,s.[ImageCreationDate])' + N', ' + CASE WHEN COL_LENGTH(N'DWH.FactPatientImage',N'DimCourseID') IS NOT NULL THEN N'TRY_CONVERT(bigint,s.[DimCourseID])' ELSE N'CAST(NULL AS bigint)' END + N', ' + CASE WHEN COL_LENGTH(N'DWH.FactPatientImage',N'ctrImageSer') IS NOT NULL THEN N'TRY_CONVERT(bigint,s.[ctrImageSer])' ELSE N'CAST(NULL AS bigint)' END + N', ' + CASE WHEN COL_LENGTH(N'DWH.FactPatientImage',N'FactPatientImageID') IS NOT NULL THEN N'TRY_CONVERT(bigint,s.[FactPatientImageID])' ELSE N'CAST(NULL AS bigint)' END + N', ' + CASE WHEN COL_LENGTH(N'DWH.FactPatientImage',N'ImageId') IS NOT NULL THEN N'TRY_CONVERT(nvarchar(255),s.[ImageId])' ELSE N'CAST(NULL AS nvarchar(255))' END + N', ' + CASE WHEN COL_LENGTH(N'DWH.FactPatientImage',N'ImageType') IS NOT NULL THEN N'TRY_CONVERT(nvarchar(255),s.[ImageType])' ELSE N'CAST(NULL AS nvarchar(255))' END + N', ' + CASE WHEN COL_LENGTH(N'DWH.FactPatientImage',N'ExposureTime') IS NOT NULL THEN N'TRY_CONVERT(float,s.[ExposureTime])' ELSE N'CAST(NULL AS float)' END + N' FROM DWH.FactPatientImage s WHERE s.ImageCreationDate>=@a AND s.ImageCreationDate<DATEADD(day,1,@b)';
