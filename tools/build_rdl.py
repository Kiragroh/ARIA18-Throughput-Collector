...
def _tablix(name: str, top: float) -> tuple[str, float]:
    ...
    hidden = ""
    if name in {"SessionDetails", "AppointmentDetails", "EventDetails", "ImageObjects", "ImageAcquisition"}:
        hidden = "<Visibility><Hidden>=Not(Parameters!IncludePseudonymizedDetails.Value)</Hidden></Visibility>"
    if name in {"SessionDetails", "AppointmentDetails", "EventDetails", "ImageObjects", "ImageAcquisition"}:
        date_field = "event_start" if name in {"EventDetails", "ImageObjects", "ImageAcquisition"} else "service_date"
