"""Optional aggregate diagnostics of native ARIA waiting-area evidence."""
FIELDS=['source_state','period','cohort','quality','appointments','patients','median_minutes']
REQUIRED={
 'ScheduledActivity':['ScheduledActivitySer','PatientSer','ScheduledStartTime','ActualStartDate'],
 'PatientLocation':['ScheduledActivitySer','CheckedInFlag','ArrivalDateTime'],
 'PatientLocationMH':['ScheduledActivitySer','CheckedInFlag','ArrivalDateTime'],
 'Patient':['PatientSer','FirstName','LastName'],
}
...
def query():
    try:
        ...
        from build_collector_v2 import column_available,lit
    missing=' OR '.join('NOT '+column_available('dbo.'+t,c) for t,cs in REQUIRED.items() for c in cs)
    empty="SELECT N'UNAVAILABLE' AS source_state,CAST(NULL AS nvarchar(7)) AS period,CAST(NULL AS nvarchar(32)) AS cohort,CAST(NULL AS nvarchar(32)) AS quality,CAST(NULL AS int) AS appointments,CAST(NULL AS int) AS patients,CAST(NULL AS float) AS median_minutes;"
    return ('SET NOCOUNT ON; IF '+missing+' BEGIN '+empty+' RETURN; END; '
            'BEGIN TRY DECLARE @sql nvarchar(max)='+lit(body())+
            "; EXEC sys.sp_executesql @sql,N'@a date,@b date',@a=@PeriodStart,@b=@PeriodEnd; END TRY BEGIN CATCH "+empty+' END CATCH;')
