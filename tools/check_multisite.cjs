...
(async()=>{
 ...
 try{
  ...
  if(await page.locator('#sources tbody tr').count()!==count)throw Error('Source audit table');
  ...
  if(legacy&&!(await page.locator('#actions tbody tr').count()))throw Error('Missing review actions');
